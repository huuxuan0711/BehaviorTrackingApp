...
data class GetDashboardDataParams(
    ...
    val queryMode: DashboardQueryMode = DashboardQueryMode.CalendarDay
)

sealed interface DashboardQueryMode {
    data object CalendarDay : DashboardQueryMode
    data object Sliding24Hours : DashboardQueryMode
}
    ...
    suspend operator fun invoke(params: GetDashboardDataParams): GetDashboardDataOutcome {
        ...
        val windowStartMillis = runStage(DashboardDataStage.RESOLVE_DATE, params) {
            when (params.queryMode) {
                DashboardQueryMode.Sliding24Hours -> Instant.ofEpochMilli(params.nowMillis)
                    .atZone(zoneId)
                    .withMinute(0)
                    .withSecond(0)
                    .withNano(0)
                    ...
                    .toInstant()
                    .toEpochMilli()
                DashboardQueryMode.CalendarDay -> params.nowMillis - 24L * 60L * 60L * 1000L
            }
        }.getOrElse { return GetDashboardDataOutcome.Failure(it.toDashboardError(params.timezoneId)) }
        ...
        val hourlyUsage = runStage(DashboardDataStage.BUILD_HOURLY_USAGE, params) {
            when (params.queryMode) {
                ...
                DashboardQueryMode.CalendarDay -> aggregator.buildHourlyUsage(
